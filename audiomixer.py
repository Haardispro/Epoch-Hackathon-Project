...
def open_html():
    os.system('brave-browser index.html')


def download_background_music():
    y=background_music_entry.get()
    results = YoutubeSearch(y, max_results=1).to_json()
    ...
    video = bg_music.streams.filter(only_audio=True).first()
    destination = "temp/"
...
def download_vocal():
    ...
    results = YoutubeSearch(x, max_results=1).to_json()
    results_dict = json.loads(results)

    for v in results_dict['videos']:
        x = 'https://www.youtube.com' + v['url_suffix']
        vocals_music = YouTube(x)

    video = vocals_music.streams.filter(only_audio=True).first()
    destination = "temp/"
    out_file = video.download(output_path=destination)
    base, ext = os.path.splitext(out_file)
